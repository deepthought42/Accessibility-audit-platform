...
/**
 * Applies the {@code consumer=page-builder} common tag to every metric when
 ...
 *
 * <p>Guarded by {@link ConditionalOnBean}: if PageBuilder is deployed without
...
public class BrowsingClientMetricsConfig {
    ...
    public void applyConsumerCommonTag() {
        meterRegistry.config().meterFilter(MeterFilter.commonTags(Tags.of("consumer", "page-builder")));
    }
}
